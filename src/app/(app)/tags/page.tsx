...
/**
 * Gestão de tags num lugar só, em vez de uma linha por tag na sidebar —
 * escala melhor quando tem muitas, e não vira uma pilha de ícones sem
 * rótulo quando a sidebar está colapsada.
 */
export default function TagsPage() {
  ...
  async function createTag() {
    ...
    setCreating(true);
    try {
      await postJSON("/api/tags", { name: newName.trim() });
  ...
  async function saveEdit(tag: TagDTO) {
    ...
    setSavingEdit(true);
    try {
      await patchJSON(`/api/tags/${tag.id}`, { name });
      await mutate("/api/tags");
      setEditingId(null);
    } catch (err) {
              ...
              {editingId === tag.id ? (
                ...
                <>
                  ...
                  >
                    ...
                    {typeof tag._count?.notes === "number" && (
