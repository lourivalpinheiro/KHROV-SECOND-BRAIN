...
/**
 * Toolbar enxuta pro editor de sermão — só o básico de texto (sem
 * flashcard/wikilink/tabela/imagem, que não fazem sentido aqui). Ver
 * src/components/editor/toolbar.tsx pra versão completa usada nas notas.
 */
export function SermonToolbar({ editor }: { editor: Editor | null }) {
  ...
  return (
    <div className="flex items-center gap-0.5 overflow-x-auto border-b bg-background/60 px-2 py-1.5 [&>*]:shrink-0">
      ...
      <Item label="Citação (versículo)" active={editor.isActive("blockquote")} onClick={() => editor.chain().focus().toggleBlockquote().run()}>
        <Quote />
      </Item>
